...
use crate::generic::controllers::crud_query::{FieldKind, FieldSpec};
...
) -> Vec<crate::generic::views::stats::CategoricalValueCount> {
    fields
        .iter()
        .flat_map(|&field| {
            ...
            counts.into_iter().map(move |(value, count)| {
                crate::generic::views::stats::CategoricalValueCount {
                    field,
                    value,
                    count,
                }
